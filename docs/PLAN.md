...
page — [`docs/editor-guide.md`](editor-guide.md) — and a short recording linked
from it, as backup, neither the primary route. The recording is a narrated
walkthrough of the same ground as the guide, hosted on Drive under the club's
own **`hkucc.web@gmail.com`** identity rather than committed to the repository
— a video does not belong in git history, and the account matters for the same
reason every other service here uses it: handover is a credential handoff, not
a migration. Shared as "anyone with the link", not restricted to named
collaborators, because a committee that turns over every year should not have
to re-grant Drive access to whoever's turn it is.
